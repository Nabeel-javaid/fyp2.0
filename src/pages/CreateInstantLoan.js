    ...
    const getCoins = async () => {
      try {
        ...
        for (let token of nonZeroBalances) {
          ...
          // Game
          const coinLogo = COINS_LIST.tokens.find((taken) => taken.address === token.contractAddress);
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <FormControl fullWidth>
                  <InputLabel style={{ fontWeight: 'normal', marginLeft: '-2px' }}>Collateral Address</InputLabel>
                  <Select
                    value={lendingToken}
                    onChange={(e) => {
                ...
                <Grid item xs={12}>
                  <FormControl fullWidth>
                    <InputLabel
                      style={{ fontWeight: 'normal', marginLeft: '-2px', }}
                    >
                      {userNFTs.length === 0 ? "No NFTS Found" : "Lending NFT"}
                    </InputLabel>
                    <Select
                      value={tokenId}
                      onChange={(e) => setTokenId(e.target.value)}
                      label={userNFTs.length === 0 ? "No NFTS Found" : "Lending NFT"}
                    >
